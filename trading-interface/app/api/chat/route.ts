...
import { setChainId } from '@/utils/getClients';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.log('Context messages (last 4 messages):');
    const last4Messages = contextMessages.slice(-4)
    console.dir(last4Messages, { depth: null })
    // JSON.stringify(contextMessages.slice(-4), null, 2)
